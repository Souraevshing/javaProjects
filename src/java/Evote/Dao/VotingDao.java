...
public class VotingDao {
    ...
    ps2=DBConnection.getConnection().prepareStatement("select candidate_id,username,party,symbol from user_details,candidate where user_details.aadhar_no=candidate.userid and candidate_id=?");
    ps3=DBConnection.getConnection().prepareStatement("insert into voting values(?,?)");
    ps4=DBConnection.getConnection().prepareStatement("select candidate_id,username,party,symbol from user_details,candidate where user_details.aadhar_no=candidate.userid and candidate.city=(select city from user_Details where aadhar_no=?)");
    ...
    ps8=DBConnection.getConnection().prepareStatement("select gender,count(*) as vote from user_Details,voting where voting.voter_id=user_Details.aadhar_no group by gender order by 2 desc");
        }
